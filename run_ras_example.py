...
async def main():
    ...
    if download_one:
        ...
        async with RasBrowser() as rb:
            ...
            # Используем RasDownloader для загрузки PDF
            ...
            try:
                ...
                if content and content.startswith(b"%PDF") and len(content) > 1000:
                    ...
                    print(f"[SUCCESS] PDF saved via RasDownloader to {out_path}")
                    saved = True
                else:
                    print(f"[ERROR] Downloaded content is not a valid PDF (size: {len(content) if content else 0})")
            except Exception as e:
                print(f"[ERROR] RasDownloader failed: {e}")
            
            if keep_open and not saved:
                print("[INFO] Keeping page open for manual inspection")
